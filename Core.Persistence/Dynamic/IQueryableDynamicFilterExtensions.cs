...
/*
Bu sınıf, `IQueryable` sorgularını dinamik olarak filtrelemek ve sıralamak için uzantı metotları sağlar. Kullanıcıdan gelen dinamik sorgu kriterlerine göre veritabanı sorgularını esnek şekilde oluşturur.### Temel İşlevler:
    ToDynamic: Dinamik sorgu (`DynamicQuery`) alarak sorguya filtreleme ve sıralama uygular.
    Filter: Verilen filtre kriterlerine göre sorguya `where` koşulu ekler.
    Sort: Verilen sıralama (`Sort`) kriterlerine göre sorguyu sıralar.
    Transform: Filtre kriterlerini uygun SQL/LINQ ifadesine dönüştürür.
Bu yapı, özellikle kullanıcı girdilerine göre sorgu oluşturma gereken uygulamalarda (örneğin, arama ve sıralama özellikleri olan tablolar) faydalıdır. 
*/
